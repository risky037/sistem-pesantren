...
export default function SantriLayout({ children }) {
    ...
    const getPageTitle = () => {
        ...
        const segments = cleanUrl.replace(/^\/santri\/?/, '').split('/').filter(Boolean);
        ...
        <div className="h-screen bg-gray-100 flex overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
                ...
                <header className="bg-white shadow-sm px-4 sm:px-6 py-3 flex items-center justify-between sticky top-0 z-10">
                    {/* Hamburger (mobile) */}
                    <button
                        id="santri-sidebar-toggle"
                        onClick={() => setSidebarOpen(true)}
                        className="lg:hidden p-2 rounded-md text-gray-500 hover:text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-emerald-500"
                    ...
                    <div className="flex items-center gap-2 ml-auto pl-4 min-w-0">
                        ...
                        <div className="h-8 w-8 rounded-full bg-emerald-600 text-white text-sm font-bold flex items-center justify-center flex-shrink-0 uppercase">
