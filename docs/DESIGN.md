...
*   **Santri Authentication Contract (P0-1B.4):** Santri possess an authenticated login identity via `santris.user_id` (FK to `users`). Admin provisions both the authentication identity (`users`) and the academic record (`santris`) atomically. The Santri portal itself (schedule viewer, material downloads) is planned for P1-1.
...
*   **Consolidated Dashboard Destination:** `UserRole::dashboardRouteName(): ?string` maps `Admin` to `'admin.dashboard'`, `Ustadz` to `'ustadz.dashboard'`, and `Santri` to `'santri.dashboard'` (stub route for routing completion; full portal planned for P1-1).
*   **Fail-Closed Security Invariant:** Any login attempt with an unrecognized or malformed role is terminated fail-closed: the user session is logged out, the session is invalidated, the CSRF token is regenerated, and an HTTP 403 response is returned. Valid roles (Admin, Ustadz, Santri) resolve to their respective dashboard routes.
*   **Credential Lifecycle (P0-1B.2/1B.3):** Self-service password reset and email verification routes have been removed. Account recovery is an Admin-only operation via dedicated Admin reset routes. The `is_active` flag gates both login and mid-session access; inactive users are rejected at the authentication boundary.
